...
#define OLED_W_SCL(x)		digitalWrite(D5, (x))
#define OLED_W_SDA(x)		digitalWrite(D6, (x))
...
void OLED_ShowPointNum(uint8_t Line, uint8_t Column, float Number, uint8_t Int_Length, uint8_t Float_Length)
{
  ...
  uint32_t FloatPart = (uint32_t)((Number - IntPart) * OLED_Pow(10, Float_Length));

  OLED_ShowNum(Line, Column, IntPart, Int_Length);
  OLED_ShowChar(Line, Column + Int_Length, '.');
  OLED_ShowNum(Line, Column + Int_Length + 1, FloatPart, Float_Length);
}
...
void OLED_ShowAHanzi(uint8_t Line_Temp, uint8_t Column_Temp, uint8_t Number)
{
	...
	for (i = 0; i < 16; i++)
	{
		if (Number * 2 + 1 < sizeof(OLED_Hanzi16x16) / sizeof(OLED_Hanzi16x16[0])) // 检查数组边界
			OLED_WriteData(OLED_Hanzi16x16[Number * 2 + 1][i]);		//显示下半部分内容
	}
}
